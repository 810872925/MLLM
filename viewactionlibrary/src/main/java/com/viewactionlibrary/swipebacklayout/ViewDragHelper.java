package com.viewactionlibrary.swipebacklayout;
...
import static com.viewactionlibrary.swipebacklayout.SystemBarUtil.getNavigationHeight;
import static com.viewactionlibrary.swipebacklayout.SystemBarUtil.hasNavigationBar;

/**
 * Author: 马龙
 * E-mail: 178917169@qq.com
 * CreatedTime: 2018/2/6 17:59
 * <p>
 * ViewDragHelper
 */
public class ViewDragHelper {
    ...
    private int getEdgeTouched(int x, int y) {
        ...
        int edgeBottomSize = mEdgeSize;
        if (SDK_INT >= KITKAT && mParentView.getContext() instanceof Activity
                && hasNavigationBar(mParentView.getContext())) {
            final Activity activity = (Activity) mParentView.getContext();
            mParentView.getGlobalVisibleRect(mTempRect);
            if (mTempRect.bottom >= getNavigationHeight(activity)) {
                final int navigationHeight = getNavigationHeight(activity);
                if (navigationHeight > 0) edgeBottomSize += navigationHeight;
            }
        }
        if (y > mParentView.getBottom() - edgeBottomSize) result = EDGE_BOTTOM;
